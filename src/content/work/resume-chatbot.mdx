...
Human-facing response text for conversational flow. Great for chat, but insufficient when you need to modify structured data predictably.
...
The AI describes what it *will* do, but hasn't actually done it yet. The user must wait for the full response, then hope the subsequent action matches the description.
...
Incremental structured payloads that become valid after stabilization. The model streams fragments of the object, and the client applies them as soon as they're complete enough to be safe.

**Example 1: Streaming a text value**

The LLM streams raw text character by character. The Vercel AI SDK accumulates these into partial objects, ensuring valid JSON structure at each step:

```javascript
// Step 0: LLM streams raw text (no structure yet)
// AI SDK receives: '{ "op": "repl'
// The SDK buffers incomplete tokens and waits for valid JSON

// Step 1: Partial operation name, incomplete object
// AI SDK provides valid but incomplete structure
{ "op": "repl" }

// Step 2: Complete operation and path - patch can start applying
// From here the UI can begin showing the update
{ "op": "replace", "path": "/summary" }

// Step 3: First character of value arrives
{ "op": "replace", "path": "/summary", "value": "S" }

// Step 4: Partial word completion - user sees "Senior bac"
{ "op": "replace", "path": "/summary", "value": "Senior bac" }

// Step 5: Complete value
{ "op": "replace", "path": "/summary", "value": "Senior backend engineer with 8+ years experience" }
```

**Example 2: Streaming an object value**

When the value is an object, partial objects show incomplete nested properties:

```javascript
// Step 0: LLM streams raw text
// AI SDK receives: '{ "op": "add", "path": "/skills/-", "val'

// Step 1: Partial operation name
{ "op": "add", "path": "/skills/-" }

// Step 2: Object value starts, partial key
{ "op": "add", "path": "/skills/-", "value": { "nam": undefined } }

// Step 3: Key completes, new value starts
{ "op": "add", "path": "/skills/-", "value": { "name": "Pytho", "level": undefined } }

// Step 4: Partial word - "Pytho" becomes "Python"
{ "op": "add", "path": "/skills/-", "value": { "name": "Python", "level": "int" } }

// Step 5: Both properties complete
{ "op": "add", "path": "/skills/-", "value": { "name": "Python", "level": "intermediate" } }
```

**Key insight:** The `stabilize()` function waits until required fields are present and valid before allowing the patch to apply. In Example 1, patching can begin at Step 2 once we have complete `op` and `path`. The AI SDK handles the raw text stream and ensures we always receive valid partial objects, even when the LLM output is incomplete.
...
**Strengths:** Easy to reason about for one narrow schema; type-safe by design.

**Weaknesses:** Tool count grows with every new field pattern. Streaming contracts fragment across tools. Cross-document reuse collapses--each new document type needs its own complete tool suite.
...
**Strengths:** Clean contract for final state and strong schema guardrails.

**Weaknesses:** Poor incremental UX for large artifacts. Small edits (like adding one skill) still wait for the entire object to generate and validate. Reference implementations: [AI SDK generateObject](https://ai-sdk.dev/docs/reference/ai-sdk-core/generate-object), [Mastra structured output](https://mastra.ai/docs/agents/structured-output), [LangChain structured output](https://js.langchain.com/docs/how_to/structured_output/)
...
**Strengths:** Low-latency, path-scoped updates with deterministic apply semantics. One mutation model works across nested objects, arrays, and other structured documents. Enables true incremental rendering--users see changes as they arrive.
