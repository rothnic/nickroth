...
<div class="not-prose my-6 p-4 bg-primary/10 border-l-4 border-primary rounded-r-lg">
  <p class="font-bold text-primary mb-1">TL;DR</p>
  <p class="text-sm text-base-content/80 m-0">Instead of regenerating entire resumes, this system streams surgical JSON patches (RFC 6902) from an AI agent. Each patch targets a specific field - like changing just your job title - without touching anything else. The result: instant updates, zero formatting loss, and no document drift.</p>
</div>
