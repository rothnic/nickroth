...
| <span class="metric-label">Failure Mode</span> | <span class="metric-label">What Users Expect</span> | <span class="metric-label">What Actually Happens</span> |
|---|---:|---:|
| <span class="metric-before">Partial context windows</span> | Small localized change, global context preserved | Edit applies only to a slice, omitting linked references or reordering sections |
| <span class="metric-before">Ambiguous match targets</span> | AI updates the specific "Skills" list item | The patch selector matches a different list or team entry with similar text |
| <span class="metric-before">Patch sequencing drift</span> | Multiple small ops apply in order, formatting preserved | Newlines and list markers shift, producing malformed markdown or duplicated bullets |
| <span class="metric-before">Unstable insertion behavior</span> | New text appears smoothly in place | Streamed partial output causes the UI to jump or show placeholders, confusing the user |
| <span class="metric-before">Rich-format constraints</span> | Structured fields (dates, locations) stay typed and validated | Markdown-first edits tear structured data into free text, breaking downstream parsers |
...
Patch values can be any valid JSON payload, not only strings.
...
7. The updated artifact state streams back to the Artifact UI and synchronizes with the Chat UI so conversation and edits stay aligned (Artifact UI ↔ Chat UI).
