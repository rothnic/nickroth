...
The LLM streams character by character. The Vercel AI SDK accumulates these into partial objects:

```javascript
// Step 1: LLM streams -> AISDK receives partial object
{ "op": "replace", "path": "/summary" }

// Step 2: Value starts arriving
{ "op": "replace", "path": "/summary", "value": "S" }

// Step 3: More characters arrive
{ "op": "replace", "path": "/summary", "value": "Senior bac" }

// Step 4: Complete value
...
```javascript
// Step 1: Operation and path are clear
{ "op": "add", "path": "/skills/-" }

// Step 2: Object starts, only partial key available
...
// Step 4: Both properties complete
{ "op": "add", "path": "/skills/-", "value": { "name": "Python", "level": "intermediate" } }
```

**Key insight:** The `stabilize()` function waits until required fields are present and valid before allowing the patch to apply. This prevents broken intermediate states from reaching the UI.

This separation lets chat stay responsive while document mutation remains safe. For the full framework comparison, partial-object behavior details, and benchmark results, see [Streaming Modes and Framework Benchmarking](/work/resume-chatbot-streaming-modes-benchmarking).

```mermaid
flowchart LR
  T["Text stream: chat rendering"] --> O["Object stream: tool-call contract"]
  O --> P["Partial-object stream: incremental structured state"]
```
...
Resume data is stored as structured JSON following the JSON Resume schema (basics, work, education, skills, projects). Using this standard gives us a stable, well-known surface to target edits, reuse existing toolchains, and keep type boundaries explicit for validation.
