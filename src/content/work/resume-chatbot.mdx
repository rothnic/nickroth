...
import IntroScreenshot from '../../assets/images/projects/resume-chatbot-intro-screenshot.png';
...
<figure class="media-figure">
  <img src={IntroScreenshot.src} alt="Resume chatbot interface orientation: left = chat pane, right = artifact pane" style={{width: '100%', maxHeight: '560px', objectFit: 'cover'}} />
  <figcaption>
...
## <span class="h2-prefix">REAL-TIME COLLABORATION:</span> <span class="h2-title">Three core requirements</span>
...
## <span class="h2-prefix">STREAMING REALITY:</span> <span class="h2-title">Structure and real-time feedback</span>
...
```text
User: "Tighten my summary to highlight API work."
AI stream: "Sure. I'll update your summary to focus on API design, reliability, and performance."
// UI receives: "S", "Su", "Sur", ...
```

Why it's easy: tokens are plain text so the UI can append them immediately. No parsing, no schema checks.
...
```text
// Token stream: "{", " \"op\": ", "\"repl", "ace\"", ...
// Intermediate parse could produce an invalid or truncated object if abused
```
...
[ { "op": "replace", "path": "/summary", "value": "..." },
  { "op": "add", "path": "/skills/-", "value": { "name": "Python" } }
]
```

- Partial-object strategy: stream objects that may evolve. Gate on minimal fields (op + complete pointer + value started) and show progressive previews until the value stabilizes.

```json
{ "op": "replace", "path": "/skills/2/name" }
{ "op": "replace", "path": "/skills/2/name", "value": "Py" }
...
for await (const partial of result.partialObjectStream) {
  if (!isValidOp(partial)) continue
  if (requiresValue(partial.op) && !hasStartedValue(partial)) continue
  if (!isCompletePointer(partial.path)) continue
  applyPatch(document, partial)
}
```

**UI guidance for nested text fields**
...
- **[v0.dev](https://v0.dev/)**-Most opinionated, uses shadcn/ui components and Next.js patterns. Produces the most usable code if you want to adapt components directly.
- **[Google Stitch](https://stitch.withgoogle.com/)**-Best for whole-application iterations. Uses Gemini Nano to generate visual representations quickly, then converts to HTML that agents can integrate. Great for exploring multiple directions fast.
- **[Figma Make](https://www.figma.com/make)**-Balanced approach for component-to-page level design with interactive behaviors.
