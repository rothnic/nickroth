...
The core issue is predictability. Collaborative editing needs concise, deterministic mutations instead of broad regeneration.
...
We considered a large schema-specific tool set (`addWorkItem`, `updateSkill`, `replaceSummary`) and rejected it for scalability.

JSON Patch (RFC 6902) gives one consistent API (`add`, `replace`, `remove`, `move`, `copy`, `test`) at any object depth. Agents can reuse the same operation model instead of learning dozens of schema-specific tool signatures.
...
We separate responsibilities for reliability:
...
flowchart LR
  ...
  A -->|response stream| C1[Chat Stream] --> C2[Chat UI]
  A -->|patch stream| P1[Patch Tool] --> P2[Patch Apply] --> P3[Artifact UI]
  C2 -. sync .- P3
```
