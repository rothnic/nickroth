...
**Key insight**: The AI SDK guarantees valid JSON at each step, but it doesn't guarantee the object matches your schema. That's the implementation's responsibility. In Example 1 above, having complete `op` and `path` might be "complete enough" to start showing the edit in progress, even before the full `value` arrives.
...
## <span class="h2-prefix"><span class="h2-hash" aria-hidden="true">##</span> SOLUTION:</span> <span class="h2-title">Streaming JSON Patch with partial-object stabilization</span>
...
```json
{
  // [!section:basics] Core identity and contact information
  "basics": {
    "name": "Nick Roth",
  ...
  // [!section-end]
  // [!section:work] Professional experience with achievements
  "work": [
    {
      "name": "Acme Corp",
  ...
  // [!section-end]
  // [!section:education] Academic background and certifications
  "education": [
    {
  ...
  // [!section-end]
  // [!section:skills] Technical competencies and proficiency levels
  "skills": [
    {
      "name": "TypeScript",
      "level": "advanced",
  ...
  // [!section-end]
  // [!section:projects] Notable projects and open source contributions
  "projects": [
    {
      "name": "Resume Chatbot",
  ...
  // [!section-end]
  // [!section:awards] Recognitions and achievements
  "awards": [
    {
  ...
  // [!section-end]
  // [!section:publications] Articles, papers, and published works
  "publications": [
    {
...
1. User sends a message to the Conversation Agent, which produces the Chat Stream for UI feedback.
2. The Chat Stream emits text while the planner evaluates intent in parallel.
3. When intent requires a document change, the planner emits a Semantic Tool Call that starts the Patch Stream.
4. The Patch Tool immediately loads the document and streams a snapshot to the Artifact UI so users see the current state.
5. A nested Patch LLM generates RFC 6902 patch ops incrementally, sending Partial-Object updates.
6. Once patches stabilize, the Patch Apply step commits them to the Resume Database.
7. The updated artifact state streams back to the Artifact UI and synchronizes with the Chat UI.
...
## <span class="h2-prefix"><span class="h2-hash" aria-hidden="true">##</span> NEXT STEPS:</span> <span class="h2-title">Generative design tools for rapid UX iteration</span>
