...
```javascript
// Step 0: LLM streams raw incomplete text - no valid JSON yet
...
// Step 1: Partial operation name - AI SDK produces valid but incomplete structure
{ "op": "repl" }

// Step 2: Complete op + path - this is where we can start applying the patch
// The stabilizer gates on complete op and path before allowing apply
{ "op": "replace", "path": "/summary" }

// Step 3: First character of value arrives
{ "op": "replace", "path": "/summary", "value": "S" }

// Step 4: Value builds character by character - user sees "Senior bac" live
...
```javascript
// Step 0: LLM streams raw incomplete text
// Raw LLM output: '{ "op": "add", "path": "/skills/-", "val'
// Again - no closing braces, the AI SDK reconstructs structure as tokens arrive

// Step 1: Partial operation name - apply still blocked
{ "op": "add", "path": "/skills/-" }

// Step 2: Object value starts, partial key - still blocked (incomplete structure)
{ "op": "add", "path": "/skills/-", "value": { "nam": undefined } }

// Step 3: Key completes, second property starts
{ "op": "add", "path": "/skills/-", "value": { "name": "Pytho", "level": undefined } }

// Step 4: Both keys present, values streaming in
{ "op": "add", "path": "/skills/-", "value": { "name": "Python", "level": "intermedia" } }

// Step 5: Both properties complete - patch applies
